...
class DocGen_WPClass_Settings_Page extends DocGen_WPClass_Admin_Page {
     ...
     */
    private function render_dip_format_settings($settings) {
        // Include general settings template
        require ASOSIASI_DIR . 'admin/views/dip-format-settings.php';
    }
     ...
     * @param array $settings Current settings
     */
    private function render_dip_directory_settings($settings) {
        // Include directory settings template
        require ASOSIASI_DIR . 'admin/views/dip-directory-settings.php';
    }    
    /**
     * Render template settings tab
     * @param array $settings Current settings
     */
    private function render_dip_template_settings($settings) {
        require ASOSIASI_DIR . 'admin/views/dip-template-settings.php';
    }
     ...
     */
    private function render_dip_template_list($template_dir) {
    ...
    */
    ...
    public function render() {
        // Get current settings and handle submissions
        $settings = get_option('docgen_implementation_settings', array());
        $submission_result = $this->handle_submissions();
        $current_tab = isset($_GET['tab']) ? sanitize_key($_GET['tab']) : 'directory';
        ...
        // Display submission results/errors
        if (is_wp_error($submission_result)) {
            $html .= sprintf(
                '<div class="notice notice-error"><p>%s</p></div>',
                esc_html($submission_result->get_error_message())
            );
        }
        ...
        $tabs = array(
            'directory' => __('Directory', 'docgen-implementation'),
            'templates' => __('Templates', 'docgen-implementation'),
            'format'    => __('Format', 'docgen-implementation')
        );
        ...
        $html .= sprintf(
            ...
            wp_nonce_field('docgen_implementation_settings', 'docgen_implementation_settings_nonce', true, false)
        );
        ...
        switch ($current_tab) {
            case 'templates':
                $this->render_dip_template_settings($settings);
                break;
            case 'format':
                $this->render_dip_format_settings($settings);
                break;
            case 'directory':
            default:
                $this->render_dip_directory_settings($settings);
                break;
        }
        $html .= ob_get_clean();
